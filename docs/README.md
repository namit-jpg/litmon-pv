# LitMon-PV documentation index

| Document | Audience | Description |
|----------|----------|-------------|
| [../README.md](../README.md) | Everyone | Project overview, quick start |
| [SETUP.md](SETUP.md) | Developers | Install on a new machine (Windows/macOS/Linux) |
| [ARCHITECTURE.md](architecture.md) | Engineers | Pipeline and service layout |
| [API.md](API.md) | Frontend / integrators | REST API map |
| [DEVELOPMENT.md](DEVELOPMENT.md) | Contributors | Local workflow, tests, conventions |
| [AGENTS.md](AGENTS.md) | AI agents | Handoff context for coding agents |
| [pubmed.md](pubmed.md) | Backend | NCBI E-utilities integration |
| [model_card.md](model_card.md) | PV / ML | Screening model governance |
| [pilot_runbook.md](pilot_runbook.md) | PV ops | Day-to-day pilot operations |
| [parallel_run_protocol.md](parallel_run_protocol.md) | PV Lead | Sensitivity parallel-run process |
| [CHANGELOG.md](CHANGELOG.md) | Everyone | Notable version history |

## Status

**Phase 1 pilot** — functional end-to-end system for one product line, PubMed-only literature source, human-in-the-loop decisions, export-only case handoff.

**Not** GxP validated. Production requires Phase 2 CSV / SOP / Quality audit.
